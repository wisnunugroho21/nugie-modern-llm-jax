...
class MultiHeadLatentAttentionV1(nnx.Module):
    def __init__(
        ...
        self.kv_compress_nn = nnx.Linear(self.embed_dim, self.kv_compress_dim, rngs=rngs)
        self.query_compress_nn = nnx.Linear(self.embed_dim, self.query_compress_dim, rngs=rngs)

        self.key_decompress_nn = nnx.Linear(self.kv_compress_dim, self.num_heads * self.head_dim, rngs=rngs)
        self.value_decompress_nn = nnx.Linear(self.kv_compress_dim, self.num_heads * self.head_dim, rngs=rngs)
        self.query_decompress_nn = nnx.Linear(self.query_compress_dim, self.num_heads * self.head_dim, rngs=rngs)
...
class MultiHeadLatentAttentionV4(nnx.Module):
    def __init__(self, d_model: int, d_latent: int, num_heads: int, rngs: nnx.Rngs):
        ...
        if d_latent % num_heads != 0:
            raise ValueError(f"d_latent ({d_latent}) must be divisible by num_heads ({num_heads}).")
    ...
    def __call__(self, x: jax.Array, mask: jax.Array | None = None) -> jax.Array:
        ...
        q_reshaped = q_latent.reshape(batch_size, seq_len, self.num_heads, self.head_dim)
        ...
        qk_t = q_heads @ jnp.swapaxes(l_kv_heads, -1, -2) # Shape: (batch, num_heads, seq_len, seq_len)
        ...
        weighted_latents_heads = a @ l_kv_heads  # Shape: (batch, num_heads, seq_len, head_dim)
        ...
        weighted_latents = weighted_latents_reshaped.reshape(batch_size, seq_len, self.d_latent)
