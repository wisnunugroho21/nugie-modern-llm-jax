...
class GroupedQueryLatentAttention(nnx.Module):
    def __init__(self, d_model: int, num_q_heads: int, num_kv_heads: int, head_dim: int, rngs: nnx.Rngs):
    ...
    def __call__(self, x: jax.Array, mask: jax.Array | None = None) -> jax.Array:
        ...
        q_reshaped = q_latent.reshape(batch_size, seq_len, self.num_q_heads, self.head_dim)
        ...
        l_kv_reshaped = l_kv.reshape(batch_size, seq_len, self.num_kv_heads, self.head_dim)
        ...
        weighted_latents = weighted_reshaped.reshape(batch_size, seq_len, self.num_q_heads * self.head_dim)
